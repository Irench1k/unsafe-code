...
- ✅ "Source precedence bugs creep in when two parts of the stack read the 'same' input from different sources."
- ❌ "Remember kids, always check where your parameters come from! This is very important!"

**Technical without Jargon-Heavy**:

- ✅ "Flask's `request.values` silently merges query and form parameters with query taking precedence."
- ❌ "The request context manager's value amalgamation methodology implements a precedential prioritization schema."

**Engaging without Trivializing**:

- ✅ "Plankton has stolen the safe password by exploiting parameter source confusion!"
- ❌ "Oopsie! Plankton did a hacking! 🤪"
- ❌ "The attacker successfully exfiltrated sensitive authentication credentials from the system."

**Complete without Verbose**:

- ✅ "Decorators that merge path and query parameters with inconsistent priority create binding drift between authorization checks and data access."
- ❌ "Sometimes when you use decorators and you have path parameters and also query parameters and they might get merged but maybe in different orders and this can cause problems with your authorization."
...
- ✅ "Authentication validates SpongeBob's credentials from query parameters, but the handler retrieves user identity from form data—creating two effective values for the same logical input."
- ❌ "The code has a security vulnerability in the authentication function."
...
- ✅ "Utility functions that merge sources create subtle inconsistencies developers rarely spot in review."
- ❌ "These utilities might cause inconsistencies that could potentially be difficult to notice."

**Code References**: Use specific file:line format

- ✅ "See [routes.py:73-92](routes.py#L73-L92)"
- ✅ "The vulnerability occurs in the `authenticate()` function (routes.py:42)"

**Lists**: Parallel structure, specific, actionable

- ✅ "Identify every lookup of the relevant key"
- ✅ "Note whether it comes from `.args`, `.form`, or `.values`"
