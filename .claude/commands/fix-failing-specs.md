...
2. Load `docs/ai/decision-trees.md` for diagnosis flow
3. Note whether tests are inherited (from spec.yml)
...
Delegate to **uc-spec-debugger** with the failure output.

For each failure, classify:

| Error Type | Likely Cause | Fix Agent |
|------------|--------------|-----------|
| "ref X not found" | Import chain or scope | uc-spec-sync or uc-spec-author |
| Assertion mismatch (inherited) | Code regression | uc-code-crafter |
| Assertion mismatch (new test) | Wrong assertion | uc-spec-author |
| Vuln test passes unexpectedly | Test too weak | uc-spec-author |
| Vuln test fails unexpectedly | Accidentally fixed | Add exclusion |

### Step 3: Apply Fixes
Based on diagnosis:

**If code issue:**
- uc-code-crafter fixes source to match intended behavior
- Compare to README and previous version

**If spec issue:**
- uc-spec-author adjusts test
- Verify against README intent

**If inheritance issue:**
- uc-spec-sync runs ucsync, updates spec.yml
- Check import chains
...
3. If still fails → check import chain with uc-spec-debugger
