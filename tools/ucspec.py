...
def rewrite_imports_for_inheritance(
    content: str, source_files: dict[str, Path], current_file_rel_path: str
) -> str:
    """
    Rewrite @import statements in inherited content to use ~ prefix for sibling files.

    When a file is inherited with a ~ prefix, its @import statements need to be updated
    to point to the renamed files. For example:
    - @import ./happy.http -> @import ./~happy.http
    - @import ../../status/patch/happy.http -> @import ../../status/patch/~happy.http
    - @import ../_imports.http -> unchanged (infrastructure files keep their names)
    - @import ../common.http -> unchanged (files outside version directory)
    """
    ...
    def rewrite_import(match: re.Match) -> str:
        ...
        # Don't rewrite infrastructure files or files that are already prefixed
        if is_infrastructure_file(filename) or filename.startswith(INHERITED_PREFIX):
        ...
        try:
            resolved_path = (current_dir / import_path).as_posix()
            ...
            # Only rewrite if the normalized path exists in source_files
...
def generate_version(
    ...
    for rel_path, source_path in source_files.items():
        ...
        source_content = rewrite_imports_for_inheritance(
            source_content, source_files, rel_path
        )
